...
public class SanPhamChiTietDao implements SanPhamChiTietInterface {
    ...
    public List<SanPhamChiTiets> getAll() {
        ...
        sql = "SELECT SanPham.MaSP, IMEI.Ma, SanPham.Ten,  SanPhamChiTiet.SoLuong, SanPhamChiTiet.Gia, RAM.dungLuong, CONCAT(CPU.hangCPU + ' ', CPU.congNgheCPU + ' ', CPU.loaiCPU),\n"
                + "HeDieuHanh.phienBan, MauSac.tenMauSac, SanPhamChiTiet.TrangThai, SanPhamChiTiet.MoTa FROM SanPhamChiTiet\n"
                + "INNER JOIN SanPham ON SanPhamChiTiet.sanphamid = SanPham.SanPhamId\n"
                + "INNER JOIN RAM ON SanPhamChiTiet.ramId = RAM.ramId \n"
                + "INNER JOIN OCung ON SanPhamChiTiet.ocungId = OCung.ocungId\n"
                + "INNER JOIN MauSac ON SanPhamChiTiet.mauSacId = MauSac.mauSacId \n"
                + "INNER JOIN HeDieuHanh ON SanPhamChiTiet.HeDieuHanhId = HeDieuHanh.HeDieuHanhId\n"
                + "INNER JOIN CPU ON SanPhamChiTiet.cpuId = CPU.cpuId\n"
                + "INNER JOIN IMEI ON SanPhamChiTiet.SanPhamChiTietId = IMEI.SanPhamChiTietId";
        try {
            con = DBConnection.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                SanPhamChiTiets s = new SanPhamChiTiets(rs.getString(1), rs.getString(3), rs.getString(2), rs.getInt(4), rs.getInt(5), rs.getString(6), rs.getString(7), 
