...
import com.acerstore.app.services.CPUServices;
...
public class AttributeCPU extends JFrame {
    ...
    CPUServices service = new CPUServices();
    ...
    private void showModel() {
        ...
        pnlLeft.add(new JScrollPane(tblCPU));
    }
    ...
    private void buttonEvent() {
        ...
        btnDelete.addActionListener((e) -> {
            try {
                int rows = tblCPU.getSelectedRow();
                int index = Integer.parseInt(tblCPU.getValueAt(rows, 0).toString());
    ...
    private void fillToTable(List<CPUModel> cpu) {
        model = (DefaultTableModel) tblCPU.getModel();
        model.setRowCount(0);
        for (CPUModel c : cpu) {
            model.addRow(c.toDataRows());
        }
    }
